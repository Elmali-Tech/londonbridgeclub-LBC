...
export default function CustomerPoolPage() {
  ...
  const [stageChangingId, setStageChangingId] = useState<number | null>(null);
  ...
  const handleStageChange = async (id: number, newStage: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setOpportunities((prev) => prev.map((o) => (o.id === id ? { ...o, deal_stage: newStage } : o)));
      } else {
  ...
  const opportunitiesByStage = useMemo(() => {
    const groups: Record<string, CustomerOpportunity[]> = {};
    PIPELINE_STAGES.forEach((stage) => { groups[stage] = []; });
    filteredOpportunities.forEach((opp) => {
      const stage = normalizeStageLabel(opp.deal_stage);
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex items-center gap-3">
             ...
             <select
              ...
              <div className="flex bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-1 rounded-2xl shadow-sm">
                <button
                  onClick={() => setViewMode("board")}
                  className={`flex items-center gap-2 px-4 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${
                    viewMode === "board" ? "bg-amber-500 text-black shadow-sm" : "text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                  }`}
                >
                  <FiColumns /> Board
                </button>
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${
                    viewMode === "grid" ? "bg-amber-500 text-black shadow-sm" : "text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                  }`}
                >
                  <FiGrid /> List
                </button>
        ...
        {filteredOpportunities.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
            {PIPELINE_STAGES.map((stage) => (
              <div key={stage} className="bg-gray-50/50 dark:bg-gray-900/40 rounded-[1.75rem] border border-gray-100 dark:border-gray-800 p-4 flex flex-col gap-3 min-h-[200px]">
                <div className="flex items-center justify-between px-2">
                  <h4 className="text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">{stage}</h4>
                  <span className="text-[10px] font-black bg-gray-200 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-full px-2 py-0.5">
                    {opportunitiesByStage[stage]?.length || 0}
                  </span>
                </div>
                <div className="flex flex-col gap-3">
                  {(opportunitiesByStage[stage] || []).map((opp) => (
                    <div key={opp.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-4 shadow-sm">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest border ${getStatusColor(opp.status)}`}>{opp.status}</span>
                        <div className="flex gap-1">
                          <button onClick={() => handleEdit(opp)} className="p-1.5 text-gray-400 hover:text-amber-500 transition-colors"><FiEdit3 size={13} /></button>
                          {canDelete && <button onClick={() => handleDelete(opp.id)} className="p-1.5 text-gray-400 hover:text-rose-500 transition-colors"><FiTrash2 size={13} /></button>}
                        </div>
                      </div>
                      <p className="font-black text-sm text-gray-900 dark:text-white leading-tight mb-1">{opp.opportunity_title}</p>
                      <p className="text-[11px] font-bold text-gray-400 mb-3">{opp.company_name}</p>
                      <p className="text-xs font-black text-amber-500 mb-3">{opp.estimated_deal_size || "N/A"}</p>
                      <select
                        value={normalizeStageLabel(opp.deal_stage)}
                        disabled={stageChangingId === opp.id}
                        onChange={(e) => handleStageChange(opp.id, e.target.value)}
                        className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl text-[10px] font-black uppercase tracking-wider outline-none focus:ring-2 focus:ring-amber-500 dark:text-white disabled:opacity-50"
                      >
